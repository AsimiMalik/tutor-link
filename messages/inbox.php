...
<div class="page">
    <div class="container" style="max-width:1000px">
        ...
        <?php if(!empty($_SESSION['flash_success'])): ?>
            <div style="background:#e6ffed;padding:10px;border-radius:6px;color:#155724;margin-bottom:12px"><?php echo htmlspecialchars($_SESSION['flash_success']); unset($_SESSION['flash_success']); ?></div>
        <?php endif; ?>
                    ...
                    <?php foreach($sent as $m): ?>
                        <div class="message-card message-row">
                            <?php if (!empty($m['receiver_pic'])): ?>
                                <img src="/brilliance/uploads/<?= htmlspecialchars($m['receiver_pic']) ?>" alt="avatar" class="message-avatar">
                            <?php else: ?>
                                <img src="/brilliance/assets/images/logo.png" alt="avatar" class="message-avatar">
                            <?php endif; ?>

                            <div class="message-meta">
                                <div style="display:flex;justify-content:space-between;align-items:start;gap:12px">
                                    <div>
                                        <div class="message-subject"><?= htmlspecialchars($m['receiver_name'] ?: 'User') ?></div>
                                    </div>
                                    <div style="text-align:right;color:#8891a6;font-size:13px">
                                        <?= htmlspecialchars($m['created_at']) ?>
                                    </div>
                                </div>
                                <div class="message-snippet" style="margin-top:8px"><?= htmlspecialchars(mb_strimwidth(strip_tags($m['body']),0,200,'...')) ?></div>
                    ...
                    <?php foreach($inbox as $m): ?>
                        <div class="message-card message-row">
                            <?php if (!empty($m['sender_pic'])): ?>
                                <img src="/brilliance/uploads/<?= htmlspecialchars($m['sender_pic']) ?>" alt="avatar" class="message-avatar">
                            <?php else: ?>
                                <img src="/brilliance/assets/images/logo.png" alt="avatar" class="message-avatar">
                            <?php endif; ?>
                            ...
                            <div class="message-meta">
                                <div style="display:flex;justify-content:space-between;align-items:start;gap:12px">
                                    <div>
                                        <div class="message-subject"><?= htmlspecialchars($m['sender_name'] ?: 'User') ?></div>
                                    </div>
                                    <div style="text-align:right;color:#8891a6;font-size:13px">
                                        <?= htmlspecialchars($m['created_at']) ?>
